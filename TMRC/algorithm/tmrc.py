...
class RECL_MIX(nn.Module):
    def __init__(self, args):
        ...
        self.attention_net = TransformerDecoder(role_heads=4, args=args)
        #
        # self.attention_net = MultiHeadAttention(args.n_heads, args.att_dim, args.att_out_dim, args.soft_temperature,
        #                                         self.dim_q, args.role_embedding_dim, args.role_embedding_dim)
...
class RECL_NET(nn.Module):
    ...
    def batch_role_embed_forward(self, batch_o, batch_a, max_episode_len, detach=False):
        ...
        for t in range(max_episode_len + 1):  # t = 0,1,2...(max_episode_len-1), max_episode_len
            ...
            agent_embedding = agent_embedding.reshape(batch_o.shape[0], self.N,
                                                      -1)  # shape=(batch_size,N, agent_embed_dim)
            agent_embeddings.append(agent_embedding.reshape(batch_o.shape[0], self.N, -1))
        # Stack them according to the time (dim=1)
        agent_embeddings = torch.stack(agent_embeddings, dim=1).reshape(-1, self.N,
                                                                        self.agent_embedding_dim)  # agent_embeddings.shape=(batch_size*(max_episode_len+1)*N, agent_embed_dim)
        role_embeddings = self.role_embedding_forward(agent_embeddings, detach=False, ema=False).reshape(-1,
                                                                                                         max_episode_len + 1,
                                                                                                         self.N,
...
class ACORM_Agent(object):
    def __init__(self, args):
        ...
        self.role_parameters = list(self.RECL.role_embedding_net.parameters()) + list(
            self.RECL.agent_embedding_net.parameters())
        self.role_embedding_optimizer = torch.optim.Adam(self.role_parameters, lr=self.lr)
        self.role_lr_decay = StepLR(self.role_embedding_optimizer, step_size=self.lr_decay_steps,
        ...
        self.encoder_decoder_para = list(self.RECL.agent_embedding_net.parameters()) + list(
            self.RECL.agent_embedding_decoder.parameters())
    ...
    def train(self, replay_buffer, total_steps):
        ...
        inputs, batch_o, batch_s, batch_r, batch_a, batch_last_a, batch_avail_a_n, batch_active, batch_dw = self.get_inputs(
            batch)
        ...
        self.update_qmix(inputs, batch_o, batch_s, batch_r, batch_a, batch_last_a, batch_avail_a_n, batch_active,
                         batch_dw, max_episode_len, total_steps)
        ...
        if total_steps > 1000000:
            self.qmix_lr_decay.step_size = 400
            self.role_lr_decay.step_size = 400
    ...
    def pretrain_agent_embedding(self, replay_buffer):
        ...
        for t in range(max_episode_len):
            ...
            agent_embeddings.append(
                agent_embedding.reshape(-1, self.N, self.agent_embedding_dim))  # (batch_size, N, agent_embedding_dim)
        ...
        decoder_output = self.RECL.agent_embedding_decoder(
            agent_embeddings.reshape(-1, self.agent_embedding_dim)).reshape(-1, max_episode_len, self.N,
                                                                            self.obs_dim + self.N)
        batch_obs_hat = batch_o[:, 1:]
        agent_id_one_hot = torch.eye(self.N).unsqueeze(0).unsqueeze(0).repeat(batch_o.shape[0], max_episode_len, 1,
                                                                              1).to(self.device)
    ...
    def update_recl(self, batch_o, batch_last_a, batch_active, max_episode_len, grad):
        # if batch_active[31, max_episode_len - 1] < 0.5:
        #     return
        self.RECL.agent_embedding_net.rnn_hidden = None
        loss = 0
        labels = np.zeros((batch_o.shape[0], self.N))  # (batch_size, N)
        for t in range(30, max_episode_len):  # t = 0,1,2...(max_episode_len-1)
            with torch.no_grad():
                agent_embedding = self.RECL.agent_embedding_forward(batch_o[:, t].reshape(-1, self.obs_dim),
                                                                       batch_last_a[:, t].reshape(-1, self.action_dim),
            ...
            role_embedding_qury = self.RECL.role_embedding_forward(agent_embedding,
                                                                   detach=False,
                                                                   ema=False).reshape(-1, self.N,
                                                                                      self.role_embedding_dim)  # shape=(batch_size, N, role_embed_dim)
            role_embedding_key = self.RECL.role_embedding_forward(agent_embedding,
                                                                  detach=True,
                                                                  ema=True).reshape(-1, self.N, self.role_embedding_dim)
            logits = torch.bmm(role_embedding_qury, self.RECL.W.squeeze(0).expand(
                (role_embedding_qury.shape[0], self.role_embedding_dim, self.role_embedding_dim)))
            ...
            for idx in range(self.batch_size):  # idx = 0,1,2...(batch_size-1)
                grad_t = grad[idx, t - 30:t, :].transpose(0, 1).detach().to('cpu').numpy()
                if batch_active[idx, t] > 0.5:
                    if t % self.multi_steps == 0:
                        clusters_labels = KMeans(n_clusters=self.cluster_num).fit(grad_t).labels_  # (1,N)
                    ...
                    for j in range(self.cluster_num):  # j = 0,1,...(cluster_num -1)
                        ...
                        for anchor in label_pos:
                            loss += -torch.log(
                                exp_logits[idx, anchor, label_pos].sum() / exp_logits[idx, anchor].sum() + 1e-5)
        loss /= (self.batch_size * (max_episode_len - 30) * self.N)
        if batch_active[idx, t] > 0.5:
            # if isinstance(loss, torch.Tensor):
            print("update recl")
            self.RECL_optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.RECL_parameters, 10)
            self.RECL_optimizer.step()



    def update_qmix(self, inputs, batch_o, batch_s, batch_r, batch_a, batch_last_a, batch_avail_a_n, batch_active,
                    batch_dw, max_episode_len, total_steps):
        ...
        for t in range(max_episode_len):  # t=0,1,2,...(episode_len-1)
            q_eval = self.eval_Q_net(
                inputs[:, t].reshape(-1, self.QMIX_input_dim))  # q_eval.shape=(batch_size*N,action_dim)
        ...
        with torch.no_grad():
            q_eval_last = self.eval_Q_net(inputs[:, -1].reshape(-1, self.QMIX_input_dim)).reshape(self.batch_size, 1,
                                                                                                  self.N, -1)
            q_evals_next = torch.cat([q_evals[:, 1:], q_eval_last],
                                     dim=1)  # q_evals_next.shape=(batch_size,max_episode_len,N,action_dim)
            q_evals_next[batch_avail_a_n[:, 1:] == 0] = -999999
            a_argmax = torch.argmax(q_evals_next, dim=-1,
                                    keepdim=True)  # a_max.shape=(batch_size,max_episode_len, N, 1)
            q_targets = torch.gather(q_targets, dim=-1, index=a_argmax).squeeze(
                -1)  # q_targets.shape=(batch_size, max_episode_len, N)
        q_evals = torch.gather(q_evals, dim=-1, index=batch_a.unsqueeze(-1)).squeeze(
            -1)  # q_evals.shape(batch_size, max_episode_len, N)

        role_embeddings = role_embeddings.reshape(-1, self.N,
                                                  self.role_embedding_dim)  # shape=((batch_size * max_episode_len+1), N, role_embed_dim)

        att_eval = self.eval_mix_net.attention_net(fc_batch_s, role_embeddings).reshape(-1, max_episode_len+1, self.N * self.att_out_dim)
        with torch.no_grad():
            att_target = self.target_mix_net.attention_net(fc_batch_s, role_embeddings).reshape(-1, max_episode_len+1, self.N * self.att_out_dim)  # ((batch*max_episode_len+1), N, att_dim)->(batch, len, N*att_dim)

        q_total_eval, grad = self.eval_mix_net(q_evals, fc_batch_s[:, :-1], att_eval[:, :-1])
        q_total_target, _ = self.target_mix_net(q_targets, fc_batch_s[:, 1:], att_target[:, 1:])
        ...
        if total_steps // 20000 > self.update_count:
            self.update_recl(batch_o, batch_last_a, batch_active, max_episode_len, grad)
            self.update_count += 1
            return
